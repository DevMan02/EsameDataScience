...
grand_chalem <- formula_frames  %>%
  filter(vincitore_campionato == vincitore_gara_Monza & vincitore_gara_Monza == pilota_piu_veloce_qualifica) %>% #conto quante volte il pilota che ha vinto monza poi ha vinto il campionato
  summarise(conteggio = n())

polePosition_vittoria <-formula_frames %>%
  filter(vincitore_gara_Monza == pilota_piu_veloce_qualifica) %>% #conto quante volte il pilota che ha vinto monza poi ha vinto il campionato
  summarise(conteggio = n())
...
ggplot(formula_conteggio_lungo, aes(x = Tipologia, y = Valore, fill = Tipologia)) +
  ...
  labs(title = "Grafico a Barre per rappresentare Grand Chalem,Pole Position e vittoria e campionato piu vittoria", x = "Tipologia ", y = "Valore") +
...
ggplot(percentuali_lunghe, aes(x = Tipologia, y = Percentuale, fill = Tipologia)) +
  ...
  labs(title = "Grafico a Barre con Percentuali", x = "Tipologia", y = "Percentuale") +
  theme_bw()
